...
sanitizers[DataTypes.HASH_TABLE] = function(propPath, data, schema, $modelManager, includeUnlisted) {
    ...
    if (_.isPlainObject(schema.schema)) {
        ...
        for (var i = 0, len = keys.length; i < len; i++) {
            ...
            //do not validate data if no property schema was defined at all!
            if (!schema.schema.hasOwnProperty(name)) {
                //includeUnlisted === true if no property schema was defined but it
                //contains internal schema property defintitions (like `timestamps` etc..)
