...
function extractDefaults(schema, data, path, skipDefaults) {
    ...
    path || (path = ['defaults']);
    let defaultValue = undefined; //current schema default if any
    ...
    if (path !== undefined && schema.default !== undefined && !skipDefaults) {
        defaultValue =  dataUtils.cloneDefaults(schema.default);
        ...
        if(schema.items instanceof Array) {
            if (defaultValue instanceof Array) {
                defaultValue.forEach(function(item) {
                    if (_.isPlainObject(item) || item instanceof Array) {
                        _setRequiresMergeTargetProperty(item);
                    }
                });
            }
            ...
            if (arrayItemDefaults) {
                ...
                _setRequiresMergeTargetProperty(arrayItemDefaults);
    ...
    function setDefaultValue(path, value) {
        if (!_.has(data, path)) {
            ...
            _.setWith(data, path, value, function(parent, key, root) {
                if (   parent === undefined
                     ...
                     */
                    const obj = {};
                    _setRequiresMergeTargetProperty(obj);
                    return obj;
                }
            });
        }
 ...
 * return {undefined}
 */
function _setRequiresMergeTargetProperty(obj) {
