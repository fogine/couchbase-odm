...
function extractDefaults(schema, data, skipDefaults, path, subPath) {
    ...
    if (schema.type === 'object' && _.isPlainObject(schema.properties)) {
        Object.keys(schema.properties).forEach(function(propertyName) {
            ...
            const dataTarget = _resolveDataTarget(data, propPath, _subPath, def);
        ...
        if(schema.items instanceof Array) {
            schema.items.forEach(function(subSchema, index) {
                ...
                const dataTarget = _resolveDataTarget(data, propPath, _subPath, def);
                return extractDefaults(subSchema, dataTarget, false, propPath, _subPath);
            });
            ...
            const dataTarget = _resolveDataTarget(data, propPath, subPath, def);

            extractDefaults(schema.items, dataTarget, true, propPath, subPropPath);
        }
    }
 ...
 * @param {Array<String|Array>} path
 * @param {Object} def
 */
function _resolveDataTarget(data, path, subPath, def) {
    let p = subPath || path;
    let dataTarget = data;

    if (p.length > 1) {
        dataTarget = def.defaults;
        p.splice(0, 1);

        if (!data.includes(def)) {
            data.push(def);
        }
    }
