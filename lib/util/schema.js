...
/**
 * recursive function
 * associations are defined with `_relation` keyword:
 *
 * {
 *   user: {
 *     _relation: {type: 'User' method: 'reference'}
 *   }
 * }
 *
 * @param {Object} schema
...
function extractAssociations(schema, data, path) {
    ...
    if (_.isPlainObject(schema._relation)) {
        data.push({
            path   : path,
            type   : schema._relation.type,
            method : schema._relation.method
        });
    } else if (schema.type === 'array') {
        if (_.isPlainObject(schema.items)
            && _.isPlainObject(schema.items._relation)
        ) {
            data.push({
                path   : path,
                type   : schema.items._relation.type,
                method : schema.items._relation.method
            });
