 ...
 * @return Promise<Instance>
 */
Model.prototype.getById = Promise.method(function(id, options) {
    var key = id;
    var actions = [];
    var defaults = {
        paranoid: true,
...
function getRefDoc(options) {
    ...
    return Promise.try(function() {
        var key = self.$buildRefDocKey(options);
        return self.storage.get(key).catch(function(err) {
            if (   err instanceof StorageError
                && err.code === StorageAdapter.errorCodes.keyNotFound
            ) {
                return null;
