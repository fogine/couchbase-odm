...
Model.prototype.$init = function(modelManager) {
    ...
    var schemaData = schemaUtils.extractData(this.options.schema);

    this.relations = schemaData.relations;
    this.defaults = schemaData.defaults;
...
Model.prototype.$buildRefDocKey = function(input) {
    ...
    var key = new this.RefDocKey(options);
...
Model.prototype.build = function(data, options) {
    ...
    if (options.isNewRecord) {
        var defaultValues = _.cloneDeepWith(this.defaults, function(val) {
            if (val instanceof Instance) return val.clone();
        });

        if (this.$dataHasJsonStructure()) {
            data = _.mergeWith(defaultValues, data, function(defaultVal, val) {
                if (val instanceof Array) return val;
                //TODO tests that if val === null and defaultVal === undefined,
                //the null value is set instead of undefined
                if (val === null) return defaultVal;
            });
        } else {
            if (_.isNil(data) && this.defaults !== undefined) {
                data = this.defaults;
            }
        }
    }
...
Model.prototype.create = function(data, options) {
    ...
    return Promise.try(function() {
        ...
        var doc = self.build(data, {key: key});
...
Model.prototype.getById = function(id, options) {
    return Promise.bind(this).then(function() {
        ...
        //if paranoid option is set it will perform extra get query to check whather the doc is soft-deleted
