...
Instance.prototype.sanitize =  function(options) {
    ...
    this.setData( sanitizer.sanitize.apply(this.Model, [
                ...
                this.getData(),
                {
                    ...
                    skipInternalProperties: options.skipInternalProperties
                }
    ]));
...
Instance.prototype.update = Promise.method(function(data, options) {
    ...
    if (this.Model.$dataHasPrimitiveStructure()) {
        this.$original.setData(data);
    } else {
        _.assign(this.$original.options.data, data);
    }

    var self = this;

    return this.$original.save(options).then(function(instance) {
        if (self.Model.$dataHasPrimitiveStructure()) {
            self.setData(data);
        } else {
...
Instance.prototype.insert = function(options) {
    ...
    return this.Model.runHooks(HookTypes.beforeCreate, this, options).then(function() {
        ...
        //validate
        try {
            self.sanitize({skipInternalProperties: false});
        } catch(e) {
